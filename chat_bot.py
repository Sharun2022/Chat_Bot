    ...
    while True: #sets up while loop
        try: # While the function is true
            ...
            if num >= low and num <= high : # If input is greater or equal to low (1) and less than or equal to high (2)
                ...
                print(f"The number must be between {low} and {high}") # Asks for input again
        except ValueError:
...
# If count is greater or equal to PH_LOW (7) and less than or equal to PH_HIGH (10) 
                return str(num) # Returns and accepts input
            else: # If input is not the above
                print("NZ phone numbers have between 7 and 10 digits") # Prints message that number must be between 
        # 7 and 10 digits
        except ValueError:
...
def welcome(): #Defines following code as welcome
    ...
    Returns: None 
    '''
    num = randint(0,9) # Randomised the name of the person who will be helping the user order their food
    name = (names[num]) # Name will be chosen by their number
    print("*** Welcome to the Online Chat Bot ***") # Prints Welcome to the Online Chat Bot with asterisks
...
def order_type(): #Defines following code as order_type
    ...
    question = (f"Please enter a number between {LOW} and {HIGH} ") #Asks user to enter a number between 1 and 2
...
def delivery_info(): # Defines following code as delivery_info
    ...
    question = ("Please enter your phone number ") #Asks user to enter their phone number
    customer_details['phone'] = check_phone(question, PH_LOW, PH_HIGH) #Validates input of customer phone number
...
def menu(): # Defines following code as menu
    ...
    for count in range (number_food): #For food(s) ranged between 1 and 12
        print("{} {} ${:.2f}"  .format(count+1, food_names[count], food_prices[count])) #Print the food menu with prices
...
def order_food(): #Defines following code as order_food
    ...
    question = (f"Enter a number between {NUM_LOW} and {NUM_HIGH} ") #Asks user to enter number between 1 and inf
    print("How many food(s) do you want to order?") #Prints how many food(s) do you want to order?
    num_food = val_int(NUM_LOW, NUM_HIGH, question) #num_food equals validated input
 ...
 # Choose food from menu
    for item in range(num_food): #For food(s) between 1 and 12
        while num_food > 0: #While input is greater than 0
            ...
            food_ordered = val_int(MENU_LOW, MENU_HIGH, question) #food_ordered equals validated input  
            food_ordered = food_ordered - 1 #food_ordered equals food_ordered minus 1
            order_list.append(food_names[food_ordered]) #Food names get added to order list
            order_cost.append(food_prices[food_ordered]) #Food prices get added to order cost
            print("{} ${:.2f}" .format(food_names[food_ordered],food_prices[food_ordered]))
            #Print food(s) ordered with prices
            num_food = num_food - 1 #num_food equals num_food minus 1

# Print order out -  
# Including if order is delievery or click & collect and names and prices of each food - 
# Total cost including any delivery charge
# Takes del_pick as parameter
def print_order(del_pick): #Defines code as print_order(del_pick) with parameter
    ...
    if del_pick == "Click and Collect": # if delivery type was Click and Collect
        print ("Your Order is for Click and Collect")# Print your order is ready for click and collect
        print(f"Customer Name: {customer_details['name']} \nCustomer Phone: {customer_details['phone']}") 
        #Prints customers name and customers phone
    elif del_pick == "Delivery": # if pickup type = delivery
        print ("Your Order is for Delivery") # print your order is ready for delivery
    ...
    for item in order_list: # for the items in the order list
        print("Ordered: {}  Cost ${:.2f}".format(item, order_cost[count])) # Print customers order and cost
        count = count + 1 #Adds 1 to count
    print()# Print blank space
    if del_pick == "Delivery": # If input equals Delivery
        if len(order_list) >= 5: # If number of food ordered is more than or equal to 5
            ...
            print ("There is an additional $9.00 delivery charge") #Prints message
            total_cost = total_cost + 9  #Adds $9 to the total cost of food ordered
    ...
    if del_pick == "Click and Collect": #If input equals Click and Collect
        print("Thank you for your order, we'll let you know when it's ready") #Prints message
    elif del_pick == "Delivery": #Else if input equals delivery
        print ("Thank you for your order, it will be delievered soon")#Prints message
...
def confirm_cancel(): # Defines the following code as confirm_cancel
    question = (f"Enter a number between {LOW} and {HIGH} ") #Asks user to enter number between 1 and 2
...
# Order is confirmed with message
    ...
    if confirm == 1: # If input equals 1
        ...
        print ("Your order has been sent to our kitchen") #Prints Your order has been sent to our kitchen
...
# Order is cancelled with message           
        ...
        print ("Your Order has been Cancelled") #Prints order cancellation message
        print ("You can restart your order or exit the BOT") #Prints that they can restart the order or exit the bot
        new_exit() #Moves on to option for new order or exit the bot
...
def new_exit(): #Defines the following code as new_exit
    question = (f"Enter a number between {LOW} and {HIGH} ") #Asks user to enter between 1 or 2
    print ("Do you want to start another order? or exit?")#Prints do you want to start another order? or exit?
    print ("To start another order enter 1")#Prints to start another order, they must enter 1
    print ("To exit the BOT enter 2")#Prints to exit the bot, they must enter 2
    confirm = val_int(LOW, HIGH, question) #Confirm equals validated input
